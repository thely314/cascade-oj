...
func (_q *UserQuery) QueryJudgeRecords() *JudgeRecordQuery {
...
func (_q *UserQuery) WithJudgeRecords(opts ...func(*JudgeRecordQuery)) *UserQuery {
	query := (&JudgeRecordClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (_q *UserQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*User, error) {
	...
	if query := _q.withProblems; query != nil {
		if err := _q.loadProblems(ctx, query, nodes,
			...
			func(n *User, e *Problem) { n.Edges.Problems = append(n.Edges.Problems, e) }); err != nil {
			return nil, err
		}
	}
	if query := _q.withJudgeRecords; query != nil {
		if err := _q.loadJudgeRecords(ctx, query, nodes,
			func(n *User) { n.Edges.JudgeRecords = []*JudgeRecord{} },
			func(n *User, e *JudgeRecord) { n.Edges.JudgeRecords = append(n.Edges.JudgeRecords, e) }); err != nil {
			return nil, err
		}
	}
	...
	if query := _q.withProblemSetUsers; query != nil {
		if err := _q.loadProblemSetUsers(ctx, query, nodes,
			func(n *User) { n.Edges.ProblemSetUsers = []*ProblemSetUser{} },
			func(n *User, e *ProblemSetUser) { n.Edges.ProblemSetUsers = append(n.Edges.ProblemSetUsers, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *UserQuery) loadJudgeRecords(ctx context.Context, query *JudgeRecordQuery, nodes []*User, init func(*User), assign func(*User, *JudgeRecord)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(judgerecord.FieldUserID)
	}
	query.Where(predicate.JudgeRecord(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(user.JudgeRecordsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (_q *UserQuery) loadAnnouncements(ctx context.Context, query *AnnouncementQuery, nodes []*User, init func(*User), assign func(*User, *Announcement)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(announcement.FieldPublisherID)
	}
...
func (_q *UserQuery) loadProblemSetUsers(ctx context.Context, query *ProblemSetUserQuery, nodes []*User, init func(*User), assign func(*User, *ProblemSetUser)) error {
	fks := make([]driver.Value, 0, len(nodes))
	nodeids := make(map[int]*User)
	for i := range nodes {
		fks = append(fks, nodes[i].ID)
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(problemsetuser.FieldUserID)
	}
